...
async def comp_sync_part( i=None, mu_part=None, mu_left_part=None, mu_right_part=None, rapi=None,**kwargs):
    ...
    return {"payload":[mu_cur],"payload_info":mu_part["payload_info"],"average":part_average}
...
async def comp_step_part( mu_next_part = None, mu_part=None, i=None, expectation=None,rapi=None,**kwargs):

    #mu_cur = np.empty(Psize+1,np.float64)
    if mu_next_part is None:
        mu_next = np.zeros([Psize+2])
    else:
        mu_next = mu_next_part["payload"][0]
        
    mu_prev = mu_part["payload"][0]
    
    #mu_cur[Psize+1] = 0.0
    mu_cur, left, right = njit_comp_step_part( mu_next, mu_prev, i, expectation )
    # тоже посчитаем
    part_average = njit_comp_part_average( mu_cur, i )
    
    #d = await rapi.submit_payload( mu_cur )
    if mu_next_part is None:
        return {"payload":[mu_next],"left":left,"right":right,"average":part_average}
    print("mu_next_part =",mu_next_part,flush=True)
    return {"payload":[mu_next],"payload_info":mu_next_part["payload_info"],"left":left,"right":right,"average":part_average}
"""
async def comp_step_part2( mu_part=None, i=None, expectation_stream=None,left_stream=None, right_stream=None, rapi=None,**kwargs):
    ...
    for k in range(0,100):
      ...
      mu_cur, left, right = njit_comp_step_part( mu_cur, mu_prev, i, expectation )
      mu_prev = mu_cur
      # тоже посчитаем
      part_average = njit_comp_part_average( mu_cur, i )

      # это кстати обычные сообщения. но с префиксом снаружи добавляемым
      #await rapi.submit_to_stream("left",left)
      #await rapi.submit_to_stream("right",right)
      #await rapi.submit_to_stream("average",part_average)
      await rapi.submit_to_stream(out_left_stream,left)
      await rapi.submit_to_stream(out_right_stream,right)
      await rapi.submit_to_stream(out_average,average)

      outer_left = await rapi.get_from_stream( left_stream )
      outer_right = await rapi.get_from_stream( right_stream )
      expectation = await rapi.get_from_stream( expectation_stream )

      mu_cur[0] = outer_left
      mu_cur[N] = outer_right
    
    #d = await rapi.submit_payload( mu_cur )
    return {"payload":[mu_cur],"left":left,"right":right,"average":part_average}
"""
...
async def step_part( mu_next_part,mu_part, i, expectation):
    #print("Step_part called",i)
    return await c.exec_request( c.python(comp_step_part, mu_next_part=mu_next_part, mu_part=mu_part, i=i,expectation=expectation) )

# выполняет один шаг итерации
# выход - mu, структура из блоков
async def make_one_step(mu_next=None,mu_prev=None,expectation=None,relative_step=None, N=None, t_int=None):
    mu_unsynced = []
    #print("make_one_step called")
    # цикл по кусочкам
    for i in range(len(mu_prev)):
        next_part = await step_part( mu_next[i], mu_prev[i], i,expectation )
    ...
    p_expectation = await average( mu_unsynced )

    return mu, mu_prev, p_expectation
...
async def main():
    ...
    w1 = await s.start_workers( 1, 5, 6000 )
    ...
    #p_mu_unsynced = p_mu
    p_expectation = await average( p_mu )
    ...
    p_mu_prev = p_mu
    p_mu = [None for x in range(0,P)]
    #step( mu_prev, mu_cur, relative_step, N, x_range )
    #for t_int in range(1,T+1):
    for t_int in range(1,501):
        #if GUIT is not None and t_int % GUIT == 0:
        #    print(t_int)
        print("submitting", t_int,end="\r")
        p_mu_prev, p_mu, p_expectation = await make_one_step( mu_next=p_mu,mu_prev=p_mu_prev, expectation=p_expectation, relative_step=relative_step, N=N, t_int=t_int )
        ...
        if GUI is not None and t_int % GUI == 0:
            asyncio.create_task( animate_p( p_mu_prev, t_int ) )

        if REPORT is not None and t_int % REPORT == 0:
            asyncio.create_task( report_progress( p_mu_prev, t_int, T) )

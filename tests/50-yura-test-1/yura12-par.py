...
# 13 - убираем стадию sync.
# 12 - ускоряем суммирование average
# 11 - схема с 1 буфером mu - ну чутка
# 10 - reuse - оч хороший эффект
...
async def step_part( mu_part, i, expectation, t_int):
...
async def make_one_step(mu_prev=None,expectation=None,relative_step=None, N=None, t_int=None):
    ...
    for i in range(len(mu_prev)):
        next_part = await step_part( mu_prev[i], i,expectation, t_int )

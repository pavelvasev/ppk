...
# 17-s - делаем другую схему параллельности. mu но оставляем ожидание задач
...
async def step_part( mu_part, mu_parts, i, t_int,mu_left_part,mu_right_part):
    #print("Step_part called",i)

    return await c.exec_request( 
        c.python(comp_step_part, mu_part=c.reuse(mu_part), i=i,mu_parts=mu_parts,
            left_part=c.skip_payloads(mu_left_part),right_part=c.skip_payloads(mu_right_part)),
...
async def make_one_step(mu_prev=None,expectation=None,relative_step=None, N=None, t_int=None):
    ...
    for i in range(len(mu_prev)):
        mu_left_part = None
        if i > 0:
            mu_left_part = mu_prev[i-1]
        mu_right_part = None
        if i < P-1:
            mu_right_part = mu_prev[i+1]            
        next_part = await step_part( mu_prev[i], all, i, t_int,mu_left_part,mu_right_part )

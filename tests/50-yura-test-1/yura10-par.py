...
async def sync_part( i, mu_part, mu_left_part, mu_right_part,t_int ):
    return await c.exec_request( 
        c.python(comp_sync_part,i=i,mu_part=c.reuse(mu_part), mu_left_part=c.skip_payloads(mu_left_part),mu_right_part=c.skip_payloads(mu_right_part)),
        hint={"Z":i,"X":t_int+0.4,"type":"sync"} )
...
async def step_part( mu_next_part,mu_part, i, expectation, t_int):
    #print("Step_part called",i)
    return await c.exec_request( 
        c.python(comp_step_part, mu_next_part=c.reuse(mu_next_part,alloc=True), mu_part=mu_part, i=i,expectation=expectation),
...
async def make_one_step(mu_next=None,mu_prev=None,expectation=None,relative_step=None, N=None, t_int=None):
    ...
    for i in range(len(mu_prev)):
        next_part = await step_part( mu_next[i], mu_prev[i], i,expectation, t_int )
        mu_unsynced.append( next_part )

    # синхронизируем    
    mu = []
    for i in range(len(mu_unsynced)):
        next_part = await sync_part( i, mu_unsynced[i],mu_unsynced[i-1] if i > 0 else None, mu_unsynced[i+1] if i < P-1 else None, t_int )
        mu.append( next_part )
...
async def main():
    ...
    p_mu_prev = await make_initial_mu()
    #p_mu_unsynced = p_mu
    p_expectation = await average( p_mu,0 )
    ...
    #p_mu_prev = p_mu
    #p_mu = [c.alloc(Psize+2) for x in range(0,P)]
    #p_mu = [c.reuse(p_mu_prev[x],alloc=True) for x in range(0,P)]
    #step( mu_prev, mu_cur, relative_step, N, x_range )
    #for t_int in range(1,T+1):
    for t_int in range(1,1001):
        #if GUIT is not None and t_int % GUIT == 0:
        #    print(t_int)

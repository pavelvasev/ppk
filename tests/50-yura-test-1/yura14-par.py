...
async def comp_sync_part( i=None, mu_part=None, mu_left_part=None, mu_right_part=None, rapi=None,**kwargs):
    ...
    return {"payload":[mu_cur]}

async def sync_part( i, mu_part, mu_left_part, mu_right_part,t_int ):
    return await c.exec_request( 
        c.python(comp_sync_part,i=i,mu_part=c.reuse(mu_part), 
            mu_left_part=c.skip_payloads(mu_left_part),mu_right_part=c.skip_payloads(mu_right_part)),
...
async def njit_comp_step_part( mu_part, part_num, expectation):
    ...
    return mu_part, mu_part[1], mu_part[Psize]

async def comp_step_part( mu_part=None, i=None, expectation=None,mu_left_part=None,mu_right_part=None,rapi=None,**kwargs):

    mu_prev = mu_part["payload"][0]

    if mu_left_part is not None and i > 0:
        mu_prev[0] = mu_left_part["right"]
    if mu_right_part is not None and i < P-1:
        mu_prev[Psize+1] = mu_right_part["left"]
    
    #mu_cur[Psize+1] = 0.0
    mu_cur, left, right = njit_comp_step_part( mu_prev, i, expectation )
    # тоже посчитаем
    part_average = njit_comp_part_average( mu_cur, i )
...
async def step_part( mu_part, i, expectation, t_int, mu_left_part, mu_right_part):
    #print("Step_part called",i)
    return await c.exec_request( 
        c.python(comp_step_part, mu_part=c.reuse(mu_part), i=i,expectation=expectation,
            mu_left_part=c.skip_payloads(mu_left_part),mu_right_part=c.skip_payloads(mu_right_part)),
...
async def make_one_step(mu_prev=None,expectation=None,relative_step=None, N=None, t_int=None):
    ...
    for i in range(len(mu_prev)):
        next_part = await step_part( mu_prev[i], i, expectation, t_int, mu_prev[i-1] if i > 0 else None, mu_prev[i+1] if i < P-1 else None )
        mu_unsynced.append( next_part )

    # параллельно считаем среднее общее    
    p_expectation = await average( mu_unsynced,t_int )

    return mu_unsynced, p_expectation
...
async def main():
    ...
    for t_int in range(1,4001):
        ...
        p_mu, p_expectation = await make_one_step( mu_prev=p_mu, expectation=p_expectation, relative_step=relative_step, N=N, t_int=t_int )

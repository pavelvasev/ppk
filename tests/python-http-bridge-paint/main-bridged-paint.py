...
async def main():    
    ...
    try:
        websrv = ppk.web.Server()
        ppk.http_bridge.setup_http_ppk_bridge( rapi, websrv.app )           
        await websrv.start(port=45000)
    except BaseException:
        print(traceback.format_exc())
    except Exception:
        print(traceback.format_exc())    

    print("web server started at",websrv.url)
    #print("try curl",f"\"{websrv.url}/put?label=wrk_1&value=\\[10,20,30\\]\"")
    print("try curl",f"\"{websrv.url}/req?label=wrk_1&value=\\[10,20,30\\]\" -o image.png")

...
function main( rapi, worker_ids ) {
  ...
  let next_iter = arg => {
    ...
    function get_iter_tasks(data,runner_id) {
      let res = rapi.operation("step1", {input:data}, "js")
      return [res]
    }
    ...
    function enter_iter(data,runner_id) {
      let tasks = get_iter_tasks( data, runner_id )
      let submitted = submit_tasks( tasks, runner_id )
      let res = submitted[0]; 
      // это на самом деле признак продолжения.. 
      // но в целом мы этим займемся когда перейдем к синхронизациии
      return res
    }

    let r = enter_iter( arg.data_pr[0], arg.my_id )
    
    if (arg.N == 1000) console.time("iter-dur")

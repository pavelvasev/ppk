...
// abils-1 загружает возможности в gr-4
...
function main( rapi, worker_ids ) {
  ...
  rapi.shared("compute1/output").subscribe( vals => {
    ...
    let calc_output = vals[0] // первый элемент списка. todo - шаред переменные.
...
function main_2( rapi, worker_ids, calc_output ) {
  ...
  rapi.shared("abilities").submit({title:"График 1",msg:{label:"start_process",code:"gr1",target:"abils-1"}})

  rapi.query("start_process").done( val => {
    console.log("see msg for start_process! val=",val)
    rapi.start( val.code, val.arg, val.target)
    //rapi.start( "gr1", {worker_ids,calc_output},"abils-1")
    //return main_3( rapi, worker_ids, calc_output )
  })

/*
  rapi.shared("abilities").submit({title:"График 1",msg:{label:"add_gr1"}})
  rapi.shared("abilities").submit({title:"График 2",msg:{label:"add_gr2"}})
  
  rapi.query("add_gr1").done( val => {
    console.log("see msg for add_gr1! val=",val)
    rapi.start( "gr1",0,"abils-1")
    //rapi.start( "gr1", {worker_ids,calc_output},"abils-1")
    //return main_3( rapi, worker_ids, calc_output )
  })
*/  

  let procs = rapi.shared_list_reader("abils-1")

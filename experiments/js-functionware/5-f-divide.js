...
function main( rapi, worker_ids ) {
  ...
  */
  ...
  function apply_step_1( ctx,data )
  {
    let graph = ctx.graph
  ...
  // пока требование чтобы data размерность и runners - совпадали
  function apply_step_div( ctx,data )
  {
    let graph = ctx.graph
    ...
    for (let k=0; k<data.length; k++) {
      ...
      let res = rapi.operation("step_div", 
  ...
  let res1 = apply_step_1( ctx, p_data )
  let res2 = apply_step_1( ctx,res1 )

  let res = apply_step_div( ctx, res2 )

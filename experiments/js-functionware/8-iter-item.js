...
function main( rapi, worker_ids ) {
  ...
  */
  ...
  let next_iter = arg => {
    ...
    let rr = rapi.read_cell( arg.start_data[ arg.k ].id )
    make_step( arg.N, rr )
...
//  let res = f_1d_borders( ctx, (me,left,right) => (left + right)/2 + Math.random(1) ) ( p_data )
  ...
  // todo это надо куда-то локальизовать
  res.forEach( (output_cell, index) => {
    rapi.create_link( output_cell.id, p_data[index].id )
  })

  function start_iteration( ctx, worker_ids, N, start_data ) {
    ...
    for (let k=0; k<cnt; k++) {
       ...
       rapi.exec( 
        ...
        {arg: {k, N, P:cnt, my_id: worker_ids[k], start_data, graph: ctx.graph}, 
  ...
  let cres = start_iteration( ctx, worker_ids, n, p_data )

#!/usr/bin/env -S node
// 1 - матричный граф, параллельная версия, созданный функцией compute: 282.291ms
  ...
  */
  ...
  let next_iter = arg => {
    ...
    function get_iter_tasks(runner_id) {
      let tasks = arg.iter_graph.graph[ runner_id ]
      return tasks
    }
  ...
  function generate_graph_1( data, runners ) 
  {
    let graph = {}

    let out_cells = []
    for (let k=0; k<data.length; k++) {
      let data_arr_next = rapi.open_cell(`gg1_${k}`);
  ...
  let iter_graph = generate_graph_1( p_data, worker_ids )

  // закольцовываем граф для итераций..
  iter_graph.output.forEach( (output_cell, index) => {

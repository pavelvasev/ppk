#!/usr/bin/env -S node
// test3 - пробуем населить все задачи 4 штуки в одного робота. сравним общую скорость (хотя она д быть сравнима с последовательной)
...
function super_robot_1( rapi, id, workers,f ) {
  ...
  let r = workers.map( (x,index) => start_robot_1( rapi, workers[0],
       { index, id:`${id}/${index}`,
...
function pass_robot( rapi, id, workers,N ) {
  ...
  let count = workers.length  
  let r = workers.map( (x,index) => start_robot_2( rapi,workers[0],
       { index, id:`${id}/${index}`,

...
class ImageSaver:
    def __init__(self,target_component):
        self.target_component = target_component # признак по которому находить сущности
        self.distribution = []

    def deploy( self,workers ):
        for w in workers:
            print("deploy image_saver to worker",w.id)
            nodes = gen.node( "image_saver", target_component = self.target_component, tags=["ecs_system"])
...
class image_saver:
    def __init__(self,rapi,description,parent):
        ...
        self.system_id = "image_saver"

        gen.apply_description( rapi, self, description )

    def get_components(self):
        return ["image",self.target_component]
    ...
    def process_ecs(self,i,world):
        ...
        ents = world.get_entities_with_components("image",self.target_component,marker="image_saver",updates=["image_saver"],verbose=True)
        ...
        for entity_id in ents:
            ...
            # добавим таки что это система
            e.update_component("image_saver",dict(system=1))
            # временный хак чтобы очистить признак блокировки
            e.update_component(self.target_component,dict())
        ...
        while items_on_level > 0:
            ...
            for i in range(items_on_level):
                ...
                if items_on_level == 1:
                    ...
                    #nodes["params"]["components"]["image_saver"] = dict()
...
class image_merger:
    def __init__(self,rapi,description,parent):
        ...
        #self.local_world = description["local_systems"]
        #self.local_world.add_system( "merge", self )

        self.system_id = "merge"

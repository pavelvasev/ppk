...
def print_world( w ):
    ...
    for entity_id, e in w.entities.items():
        ...
        for component_name in sorted(e.components.keys()):
            ...
            if component_name in e.systems:
                component_name_s = component_name_s + "[S]"
            ...
            if component_name in e.pending_outputs:
                ...
                if cnt > 0:
                    ...
                    for k in e.pending_outputs[component_name].keys():
                        component_name_s = component_name_s + " wait_remote: " + k

            # есть входящие сообщения (запросы), ждет процессов чтобы пропустить их
            # пояснение - процесс заблокировал обновление компоненты в том смысле
            # что он еще не сожрал старое значение а уже прислали новое
            if component_name in e.pending_updates:
                ...
                if cnt > 0:
                    ...
                    for k in e.pending_processes[component_name]:
                        component_name_s = component_name_s + " wait_proc: " + k
...
class World:
    def __init__(self):
        ...
        # т.е. исходящие ссылки
        self.links = {}

        # таблица систем #F-SYSTEMS
        self.systems = {}
    ...
    #def add_rule( self, component_names, new_component_name, new_component_value ):
    #    self.rules.append( [component_names,new_component_name,new_component_value] )
        ...
        for r in self.rules:
            ...
            for entity_id in ent_ids:
                ...
                if not e.has_component( r.target_component_name ):
                    print("RULE: add component ",entity_id + "/" + r.target_component_name )
                    e.update_component( r.target_component_name, r.target_component_value )

    #F-SYSTEMS    
    def add_system( self, system_id, system_object ):
        self.systems[ system_id ] = system_object

    # список компонент системы
    def get_system_components( self, system_id ):
        return self.systems[ system_id ].get_components()
    ...
    # updates = список компонент которые будут обновлены (update)
    # из него вычисляется, можно ли выполнять этот поиск
    # т.к. там могут быть компоненты которые заблокированы
    ...
    def get_entities_with_components(self, *component_types, marker, updates, verbose=False):
        ...
        if marker is not None:
            ...
            # ну это идея что сущность в принципе заблокирована чтобы не произвести
            # этой же компоненты случайно
            # но с другой стороны это вопрос а зачем все процессы то блокировать
            # может можно другой обойтись...
        ...
        if marker is not None:
            ...
            # это есть отслеживание в стиле when-all
            # мы ставим маркер системы в компоненту и на следующей итерации
            # это означает что она старая и обработана
            cc = set()
            for entity_id in first_component_entities:
                e = self.get_entity( entity_id )
                ...
                for component_name in component_types:
                    if marker == component_name: # исключение - системы сами в себя отметку не ставят
                        continue
                    component = e.get_component(component_name)
                    if marker in component:
                        marker_found = True
                        if verbose:
                            print("entity",entity_id,"skipped because marker found in component",component_name,"marker=",marker)
                        break
    ...
    async def _run_loop(self):
        ...
        try:
            while self._running:
                ...
                iteration += 1
                print(f"LoopComponent: Итерация {iteration} len(LOCAL_SYSTEMS)=",len(self.local_systems),
                    "---------------------------------- ---------------------------------- >",flush=True)
                self.local_world.apply_rules()

                for s in self.local_systems:
                    self.local_world.add_system( s.system_id, s )

                for s in self.local_systems:
                    s.process_ecs( iteration, self.local_world )
                print(f"LoopComponent: Итерация {iteration} успешно завершена",
                    "---------------------------------- ---------------------------------- <",flush=True)
...
class ecs_rule:
    def __init__(self,rapi,description,parent):
        ...
        self.target_component_name = None
        self.target_component_value = {}
        gen.apply_description( rapi, self, description )
        # после этого аттрибуты выше - заполнены
        self.local_world = description["local_world"]
        self.local_world.add_rule( self )        
...
class entity:
    ...
    def setup_sync(self):
        ...
        def on_put_request(v):
            ...
            if self.has_component( component_name ):
                ...
                """
                пройти по системам, работающим с этой сущностью
                и найти среди этих систем те, которые еще не обработали эту сущность
                и записать их в список locks
                """
                locks = []
    ...
    def update_component( self,component_name, component_value ):
        ...
        if "system" in component_value:
            self.systems[ component_name ] = component_value
...
def init(*args):
    ...
    gen.register({"ecs_link":ecs_link})
    gen.register({"ecs_rule":ecs_rule})

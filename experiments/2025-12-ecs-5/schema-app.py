...
##############################
class AddRuleFeature:
    #### request
    def __init__(self,rapi):
        self.rapi = rapi        
        rapi.add_rule = self.add_rule

    def add_rule( self, component_names, target_component_name, target_component_value, workers ):
        print("ENTITY RULE",component_names,"----->",target_component_name, target_component_value)

        nodes = gen.node( "ecs_rule",
            component_names = component_names,
            target_component_name = target_component_name,
            target_component_value = target_component_value,
            tags=["ecs_system"])
        for w in workers:
            w.put( {"description":nodes,"action":"create"})


ppk.DEFAULT_EXTENSIONS["add_rule"] = AddRuleFeature
##############################
...
async def main():
    ...
    try:
        ...
        saver = plugins.common.ImageSaver("final_image")
        saver.deploy( worker_channels )

        #rapi.add_rule( ["final_image"],"image_saver",{"system":1},worker_channels )

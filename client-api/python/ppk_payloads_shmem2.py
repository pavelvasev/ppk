"""
Работа с нагрузками
- версия прямого доступа к нидам
"""
import sys
import asyncio
import json
...
class Payloads:
    ...
    async def get_payload_by_req( self, payload_info):
        ...
        def cb(result):
            ...
            arr = np.frombuffer(bytearray(bytes), dtype=dt)
            ...
            with log_time("copy arr data to arr:"):
                ...
                print("arr copied from shmem, arr=",arr)
            shm_a.close()
            shm_a.unlink()
...
class PayloadsInmem:
    def __init__(self,rapi):
        ...
        rapi.forget_payloads_inmem = self.forget_payloads
    ...
    # несколько штучек
    def submit_payloads( self, data, id ):
        res = []
        self.payloads[id] = data
        c = 0
        for d in data:
            s = self.submit_payload( d, id, c )
            c += 1
            res.append( s )

        return res

    async def forget_payloads( self, id ):        
        #pass
        # чтобы Юра-9 работало, убрать:
        if id in self.payloads:
    ...
    async def on_rapi_request( self, msg ):
        ...
        data = self.payloads[ msg["payload_id"] ][ msg["payload_index"] ]
    ...
    async def on_request(self,request):
        ...
        #parts=request.raw_path[1:].split("/")
        #id = parts[0]
        #index = int(parts[1])
        id=request.raw_path[1:]
        index = int(request.query_string)
        data = self.payloads[ id ][index]
    ...
    # пока 1 штучка
    def submit_payload( self, data,id,index ):
        print("submit_payload_inmem: data=",data,"id=",id,"index=",index)

...
feature "ppk-connect" {
  root: object url = "ws://127.0.0.1:12000" id="web-client" status="connecting" {
    let rapi_module = (import_js (resolve_url "/client-api/repr-ws-client-api.js"))
    m-eval {: id=@root.id rapi_url=@root.url rapi_module=@rapi_module output_cell=(param @root "output") status_cell=(param @root "status")|
      rapi_module.connect(id,rapi_url).then(rapi => {
        output_cell.set( rapi )
        status_cell.set( "connected" )
      })
    :}
  }
}

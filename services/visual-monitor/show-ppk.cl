...
//      if (msg.hint?.coview?.Z)
    ...
    re: reaction @q3.output {: info twork=(param @root "twork") tfinish=(param @root "tfinish") dat=(param @root "runners") state=@re |
      ...
      state.runners_prev ||= {}
      let rprev = state.runners_prev[ msg.runner_id ] || {Z: -1*(1 + Object.keys( state.runners_prev ).length) }
    ...
    cv_lines input=(read @root.tref | df_mul X=@root.tscale X2=@root.tscale) title="Отрезки зависимостей" color=(m-eval {: return [46/255.0, 109/255.0, 186/255.0] :}) radius=5

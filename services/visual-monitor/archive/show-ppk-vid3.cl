...
//      if (msg.hint?.coview?.Z)
      ...
      if (info.crit == "runner-started") {
        let runner = state.runners_prev[ msg.runner_id ]
        if (runner) { // поехали с орехами
           line.Y = runner.Y
           if(prev)
           prev.Y = runner.Y // ему тоже надо
        }
      }
    ...
    re: reaction @q3.output {: info twork=(param @root "twork") tfinish=(param @root "tfinish") dat=(param @root "runners") state=@re |
      ...
      */
      ...
      let line = {X2:x, Y2:rcur.Y, Z2:rcur.Z, X: x, Y: task.Y, Z: task.Z, R:task.R, G:task.G,B:task.B,

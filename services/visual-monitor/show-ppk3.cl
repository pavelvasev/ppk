...
/*
  Y=0 для тасков
  а для раннеров Z=0 Y=номер-раннера
*/
...
//  scene_env={ |area|  }
  {
    ...
    re: reaction @q1.output {: info dat=(param @root "tasks") tpending=(param @root "tpending") tref=(param @root "tref") twait=(param @root "twait") state=@re |
      ...
      let line = {X:x, Y:0, Z:z, ID: msg.id, TITLE: msg.hint?.text,T:t, R:RGB[0],G:RGB[1],B:RGB[2], STATE: info.crit, ...(msg.hint?.coview || {}) }
//      if (msg.hint?.coview?.Z)
    ...
    re: reaction @q3.output {: info twork=(param @root "twork") 
         tfinish=(param @root "tfinish") dat=(param @root "runners") state=@re |
      ...
      state.runners_prev ||= {}
      let rprev = state.runners_prev[ msg.runner_id ] || {Z:0, Y: (1 + Object.keys( state.runners_prev ).length) }
      let rcur = { X:x, Y:rprev.Y, Z: rprev.Z,
    ...
    runpts: cv_points input=(read @root.runners | df_mul X=@root.tscale | df_set R=1 G=1 B=1) title="Точки runner"
    ...
    let clicked_runner_info = (df-slice input=@root.runners count=1 
      start=(m-eval input=(event @runpts "click_3d" | get-value) 
        {: evt runners=@root.runners |
          console.log("click3d!",evt)
          let row = runners.get_row( evt.intersect.index )
          console.log(row)
          return evt.intersect.index
        :}) | df-convert {: df index row | return {...row,TITLE:(row.TITLE || row.STATE)} :})
    callouts radius=10
        input=(@clicked_runner_info | df_mul X=@root.tscale X2=@root.tscale) delta=[0,0,-2]

    //////////////////////////////////////////    
    // нарисуем список раннеров
    node3d title="Названия runner" {
      read @root.runners | df-split column="RUNNER_ID" | repeater { |record|
            text_sprite_one 
              text=@{: t=@record.0 | return t.split("_task_label")[0] :}
              position=@{: df=@record.1 | return [0, df.Y[0], -3] :}
              radius=10 size=20 color=[1,1,1]
      }
    }
    ...
    cv_lines input=(read @root.tref | df_mul X=@root.tscale X2=@root.tscale) title="Отрезки зависимостей"
       color=@{: return [46/255.0, 109/255.0, 186/255.0] :} radius=5

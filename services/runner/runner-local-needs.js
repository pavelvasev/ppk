...
export class TrackDeployedNeeds {
  ...
  check_resources( global_id, task_limits={} ) {
    ...
    console.log("check_resources",{global_id,task_limits,rsr_used,total:this.resources_total})
  ...
  // todo resources..
  save_expanded_need( need_id, resources={ram:1024},cleanup=()=>{} ) {
  ...
  get_usage() {
    this.resources_used ||= this.compute_usage()
    return this.resources_used
  }

  need_recompute_usage() { this.resources_used = null }
  compute_usage() {
    let acc = {}
    for (let val of this.expanded_needs.values()) {
      if (!val) {
        console.log("!!!!!!!!!!!!! compute_usage !val is empty!",val)
        continue 
      }
      for (let name in val.resources)
        {
          acc[name] ||= 0
          acc[name] += val.resources[name]
        }
    }
    return acc
  }

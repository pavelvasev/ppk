/*
   ...
   executing_requests
   executing_runners
   т.к. мы теперь не получаем runner_finished. зачем оно в принципе нужно то было?
   вести учет текущего множества раннеров и задач.. но для чего?
  ...
  estimate( task, request_needs,runner ) {
      ...
      for (let id of request_needs) {
        if (!runner_needs.has( id ))
        {
            ...
            if (id.startsWith("js-client-api:js")) {
              //console.log("js found")
              continue; // hack убираем компиляцию js как фактор планирования
            }
            //if ()

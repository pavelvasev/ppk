...
class PythonEnv:
  ...
  async def reuse_payloads( self,input_promise,input_promise_id,alloc=False ):
    p = input_promise
    id = input_promise_id
    ...
    if self.has_promise( id ):
      # итак у нас промиса эта есть в памяти раннера. стало быть ее память и вернем.

      # но фишка надо ее забыть - это важно. чтобы она не учитывалась больше в расчетах будто она есть - ее больше нет.
      # ибо мы ее поели (в этом и есть смысл reuse)
    ...
    if not alloc: # may_alloc это имеется ввиду
       # стало быть аллоцировать нельзя. стало быть надо заполучить.
